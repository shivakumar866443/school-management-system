export const schoolProfile = {
  brandName: 'Peepal Tree Schools',
  tagLine: 'One modern website foundation for any school',
  phone: '+91 98765 43210',
  email: 'admissions@schoolportal.edu',
  address: 'Main Road, Knowledge City, India',
  mapQuery: 'schools near Hyderabad India',
  adminCredentials: {
    email: 'admin@school.com',
    password: 'Admin@123'
  },
  heroImage:
    'https://images.unsplash.com/photo-1580582932707-520aed937b7b?auto=format&fit=crop&w=1800&q=80'
};

export const navItems = [
  { label: 'Home', path: '/' },
  { label: 'About', path: '/about' },
  { label: 'Dashboard', path: '/dashboard' },
  { label: 'Admissions', path: '/admissions' },
  { label: 'Careers', path: '/careers' },
  { label: 'Gallery', path: '/gallery' },
  { label: 'Academics', path: '/academics' },
  { label: 'Student Growth', path: '/student-growth' },
  { label: 'Parents', path: '/parents' },
  { label: 'Contact', path: '/contact' },
  { label: 'Admin Login', path: '/admin/login' }
];

export const stats = [
  { label: 'Students', value: '1,240' },
  { label: 'Teachers', value: '86' },
  { label: 'Avg Result', value: '92%' },
  { label: 'Parent Rating', value: '4.8/5' }
];

export const dashboardMetrics = [
  { month: 'Jan', attendance: 91, performance: 82, engagement: 76 },
  { month: 'Feb', attendance: 93, performance: 85, engagement: 79 },
  { month: 'Mar', attendance: 95, performance: 88, engagement: 83 },
  { month: 'Apr', attendance: 94, performance: 90, engagement: 86 },
  { month: 'May', attendance: 96, performance: 92, engagement: 89 },
  { month: 'Jun', attendance: 97, performance: 94, engagement: 91 }
];

export const studentPerformance = [
  { name: 'Science', value: 88 },
  { name: 'Maths', value: 92 },
  { name: 'English', value: 84 },
  { name: 'Social', value: 80 },
  { name: 'Sports', value: 90 }
];

export const admissionSteps = [
  'Submit enquiry with student and parent details',
  'School team schedules campus visit or online counselling',
  'Assessment and document verification',
  'Admission confirmation and fee setup'
];

export const careers = [
  { role: 'Primary Teacher', department: 'Foundation', type: 'Full time' },
  { role: 'Mathematics Faculty', department: 'High School', type: 'Full time' },
  { role: 'Sports Coach', department: 'Activities', type: 'Part time' },
  { role: 'Academic Coordinator', department: 'Operations', type: 'Full time' }
];

export const galleryImages = [
  'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=900&q=80',
  'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=900&q=80',
  'https://images.unsplash.com/photo-1577896851231-70ef18881754?auto=format&fit=crop&w=900&q=80',
  'https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=900&q=80',
  'https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=900&q=80',
  'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?auto=format&fit=crop&w=900&q=80'
];

export const reviews = [
  {
    name: 'Parent of Grade 6 student',
    rating: 5,
    comment: 'Teachers communicate regularly and the progress reports are easy to understand.'
  },
  {
    name: 'Parent of Grade 2 student',
    rating: 4.8,
    comment: 'The school balances academics, games, events, and child confidence very well.'
  },
  {
    name: 'Alumni Parent',
    rating: 4.9,
    comment: 'A transparent school management system helps parents stay involved.'
  }
];

export const students = [
  { id: 101, name: 'Aarav Kumar', grade: '6', parent: 'Ravi Kumar', phone: '+91 90000 11101', attendance: 96, marks: 91, status: 'Excellent' },
  { id: 102, name: 'Diya Sharma', grade: '7', parent: 'Meera Sharma', phone: '+91 90000 11102', attendance: 94, marks: 88, status: 'Very Good' },
  { id: 103, name: 'Vihaan Reddy', grade: '8', parent: 'Kiran Reddy', phone: '+91 90000 11103', attendance: 91, marks: 84, status: 'Good' },
  { id: 104, name: 'Anika Rao', grade: '5', parent: 'Suma Rao', phone: '+91 90000 11104', attendance: 98, marks: 95, status: 'Excellent' }
];

export const employees = [
  { id: 201, name: 'Priya Nair', role: 'Mathematics Teacher', department: 'High School', phone: '+91 90000 22201', experience: '8 Years', status: 'Active' },
  { id: 202, name: 'Rahul Verma', role: 'Science Teacher', department: 'Middle School', phone: '+91 90000 22202', experience: '6 Years', status: 'Active' },
  { id: 203, name: 'Lakshmi Devi', role: 'Academic Coordinator', department: 'Administration', phone: '+91 90000 22203', experience: '10 Years', status: 'Active' },
  { id: 204, name: 'Arjun Singh', role: 'Sports Coach', department: 'Activities', phone: '+91 90000 22204', experience: '5 Years', status: 'Active' }
];

export const activities = [
  'Games and sports management',
  'Teacher-child interaction tracking',
  'Parent coordination and feedback',
  'Marks and academic history',
  'Attendance and behaviour insights',
  'Events, gallery, transport and announcements'
];
